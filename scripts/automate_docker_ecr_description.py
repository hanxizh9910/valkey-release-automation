...
def update_ecr_description(json_file: str, template_file: str, output_file: str) -> None:
    try:
        with open(json_file, 'r') as f:
            data = json.load(f)
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        logging.error("Usage: python automate_docker_description.py <mode: docker|ecr> <json_file> <template_file> <output_file>")
        sys.exit(1)
    ...
    try:
        if mode == "docker":
            update_docker_description(json_file, template_file, output_file)
        elif mode == "ecr":
            update_ecr_description(json_file, template_file, output_file)
        else:
            logging.error("Invalid mode. Must be 'docker' or 'ecr'.")
            sys.exit(1)
    except Exception as e:
